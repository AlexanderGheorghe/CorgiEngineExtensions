...
public class TeleportHealth : Health
{
    ...
    [NonSerialized]public TeleportPoint TeleportPoint;

    private void Update()
    {
        if (!_controller.State.IsGrounded || TeleportPoint.TeleportPoints.Count == 0) return;
        TeleportPoint = TeleportPoint.TeleportPoints[0];
        foreach (var teleportPoint in TeleportPoint.TeleportPoints)
            if (DistanceTo(teleportPoint) < DistanceTo(TeleportPoint))
                TeleportPoint = teleportPoint;
        float DistanceTo(TeleportPoint p) => Vector2.Distance(p.Position, transform.position);
    }
    public override void Kill()
    {
        if (CurrentHealth > TeleportDamageTaken && CurrentHealth > 0 && TeleportPoint)
        {
            var position = TeleportPoint.Position;
